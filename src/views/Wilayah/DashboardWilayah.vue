<template>
    <div class="min-h-screen bg-[#f6f6f6]">
        ...
        <!-- Statistik -->
        <div class="flex items-start justify-center bg-white rounded-xl shadow px-8 py-4 mx-24 my-4">
            <div class="flex flex-col items-start justify-center mr-8">
                <div class="text-sm text-gray-500 mb-1">Total Objek:</div>
                <div class="text-4xl font-bold">{{ dashboard.totalObjek }}</div>
            </div>
            <div class="text-right">
                <div class="text-sm text-gray-500 mb-1">Pertumbuhan sepanjang tahun</div>
                <div class="flex items-center justify-end">
                    <svg class="w-5 h-5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M5 10l7-7m0 0l7 7m-7-7v18" />
                    </svg>
                    <span class="text-green-500 font-semibold text-4xl mr-1">{{ dashboard.pertumbuhan }}%</span>
                </div>
            </div>
        </div>
        ...
        <template v-if="loading">
            <div v-for="n in 3" :key="n" class="bg-white rounded-lg shadow mx-24 my-4 flex animate-pulse">
                <div class="w-32 bg-gray-200 rounded-l-lg"></div>
        ...
        <!-- List Aset -->
        <template v-else>
            <div v-for="aset in dashboard.aset" :key="aset.id" class="bg-white rounded-lg shadow mx-24 my-4 flex"
                @click="goToAsetDetail(aset.id)" style="cursor:pointer">
                <img :src="aset.foto" alt="foto aset" class="w-32 object-cover rounded-l-lg" />
                <div class="p-4 flex-1">
                    <div class="font-semibold text-lg">{{ aset.nama }}</div>
                    <div class="text-sm text-gray-500 mb-1">{{ aset.jenis }} | {{ aset.pemilik }}</div>
                    <div class="text-sm text-gray-500 mb-1">{{ aset.lokasi }}</div>
                    <div class="text-sm text-gray-400">{{ aset.alamat }}</div>
                </div>
            </div>
            <div v-if="!loading && hasFetched && dashboard.aset.length === 0"
                class="text-center text-gray-500 mt-6 text-base">
